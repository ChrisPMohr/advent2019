...
def main():
    boost = 1
    while True:
        passed, result = run_simulation(boost)
        if passed:
            break
        boost *= 2
    print("min power of 2 boost {}".format(boost))
...
def run_simulation(boost):
    print("Trying boost {}".format(boost))
    with open('input.txt', 'r') as f:
        ...
        for army in immune_armies:
            army.attack_damage += boost

        num_iters = 1
        max_iters = 4000
        while num_iters < max_iters and immune_armies and infection_armies:
            num_iters += 1
    ...
    return bool(immune_armies) and num_iters < max_iters, sum(army.units for army in immune_armies)
