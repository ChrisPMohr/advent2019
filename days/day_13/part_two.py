...
def main():
    with open('input.txt', 'r') as f:
        ...
        while True:
            ...
            output = get_tile(program, pc, relative_base, input_list)
            if output is None:
                break
            tile, program, pc, relative_base = output
            ...
            if ball is not None and paddle is not None:
                input_dir = -1 if paddle > ball else (1 if ball > paddle else 0)

        print("score:", score)


def get_tile(program, pc, relative_base, input_list):
    output = run_program(program, input_list, pc, relative_base)
    if output is None:
        return None
    else:
        x, program, pc, relative_base = output

    output = run_program(program, input_list, pc, relative_base)
    if output is None:
        return None
    else:
        y, program, pc, relative_base = output
    ...
    return (x,y,tile), program, pc, relative_base
