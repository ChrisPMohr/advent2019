...
def get_output(program, phase_settings):
    states = [(list(program), 0) for _ in range(5)]
    ...
    while True:
        try:
            for i in range(len(states)):
                program, pc = states[i]
                output, new_program, new_pc = run_program(program, inputs[i], pc)
                states[i] = (new_program, new_pc)
                if i + 1 != len(states):
...
def run_program(program, input_stream, pc):
    while True:
